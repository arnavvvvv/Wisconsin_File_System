...
/* Test 23: readdir updates atime; add/remove update parent mtime/ctime */
int main() {
  int ret;
  // Create a parent directory
  CHECK(create_dir("mnt/t23_dir"));

  struct stat p0, p1, p2, p3;
  if (stat("mnt/t23_dir", &p0) != 0) { perror("stat p0"); return FAIL; }

  // readdir should bump atime only
  sleep(1);
  DIR* d = opendir("mnt/t23_dir"); if (!d) { perror("opendir"); return FAIL; }
  struct dirent* e; while ((e = readdir(d)) != NULL) { /* drain */ }
  closedir(d);
  if (stat("mnt/t23_dir", &p1) != 0) { perror("stat p1"); return FAIL; }
  ...
  sleep(1);
  CHECK(create_file("mnt/t23_dir/child")); int fd = ret; CHECK(close_file(fd));
  if (stat("mnt/t23_dir", &p2) != 0) { perror("stat p2"); return FAIL; }
  ...
  sleep(1);
  CHECK(remove_file("mnt/t23_dir/child"));
  if (stat("mnt/t23_dir", &p3) != 0) { perror("stat p3"); return FAIL; }
